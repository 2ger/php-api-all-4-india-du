...
$orders=pdo_fetchall("select p.*,r.close from user_position p left join real_time_data r on r.stock_gid=p.stock_gid where  p.sell_order_id IS NULL and (p.profit_target_price >0  or p.stop_target_price >0)");
...
// die();
 ...
 foreach ($orders as $order){
     ...
     if($order['close']>0&&$order['close']>=$order['profit_target_price']||$order['close']<=$order['stop_target_price']){
         //止盈
