...
function permission_user_account_num($uid = 0) {
	...
	if (user_is_vice_founder($user['uid']) || !empty($user_founder_info['founder_uid'])) {
		...
		$group_num = uni_owner_account_nums($user_uid, $role);

				$uids = table('users_founder_own_users')->getFounderOwnUsersList($user_uid);
		foreach ($uids as $u_key => $u_val) {
			$own_user_group_nums = uni_owner_account_nums($u_val['uid'], ACCOUNT_MANAGE_NAME_OWNER);

			if ($u_val['uid'] == $user['uid']) {
				$current_vice_founder_user_group_nums = $own_user_group_nums;
			}
	...
	foreach ($account_all_type_sign as $sign) {
		...
		if (!empty($current_vice_founder_user_group_nums)) {
			$data['vice_founder_user_created_total'] += $current_vice_founder_user_group_nums[$sign_num];
		}
	}
