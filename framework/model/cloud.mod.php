...
define('CLOUD_GATEWAY_URL', 'https://s.qincewang.com/gateway.php');
define('CLOUD_GATEWAY_URL_NORMAL', 'https://s.qincewang.com/gateway.php');
...
function _cloud_shipping_parse($dat, $file) {
	...
	if (!is_error($ret) && is_array($ret) && !empty($ret)) {
		...
		if ($ret['errno'] == '-3') { 			$message = $ret['data'] . $message;
		}
...
function cloud_request($url, $post = '', $extra = array(), $timeout = 60) {
	...
	if (strexists($url, 's.qincewang.com')) {
		$extra = array();
	}
...
function cloud_api($method, $data = array(), $extra = array(), $timeout = 60) {
	...
	$api_url = 'https://s.qincewang.com/gateway.php';
	$method = rtrim($method, '/');
...
function cloud_build() {
	...
	$dat = cloud_request('https://s.qincewang.com/gateway.php', $pars);
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('https://s.qincewang.com/gateway.php', $pars, $headers, 300);
	if(is_error($dat)) {
...
function cloud_sms_send($mobile, $content, $postdata = array(), $custom_sign = '', $use_system_balance = false) {
	...
	$response = cloud_request('https://s.qincewang.com/gateway.php', $pars);
	if (is_error($response)) {
...
function cloud_cron_create($cron) {
	...
	$result = cloud_request('https://s.qincewang.com/gateway.php', $pars);
...
function cloud_auth_url($forward, $data = array()){
	...
	$auth_url = 'https://s.qincewang.com/index.php?c=auth&a=passport&__auth=' . $query;
	return $auth_url;
}
...
function cloud_flow_master_post($flow_master) {
	...
	$dat = cloud_request('https://s.qincewang.com/gateway.php', $pars, array(), 300);
	if(is_error($dat)) {
		return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
	}
...
function cloud_v_to_xs($url) {
	...
	cloud_request('https://s.qincewang.com/gateway.php', $pars);
	return true;
}
